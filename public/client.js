...
function worldToMM(wx, wz) {
  // Project world offset onto camera's local right/forward axes.
  // camera right   in world XZ = (cos(camYaw), -sin(camYaw))
  // camera forward in world XZ = (sin(camYaw), -cos(camYaw))
  // canvas +X = right, canvas -Y = up (forward).
  // Verified correct for all 4 cardinal yaw values.
  const dx  = wx - local.x;
  const dz  = wz - local.z;
  const cos = Math.cos(camYaw);
  const sin = Math.sin(camYaw);
  return {
    x: MM_CX + ( dx * cos - dz * sin) * MM_SCALE,
    y: MM_CY + (-dx * sin + dz * cos) * MM_SCALE
  };
}
