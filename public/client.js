...
function toggleSound() {
  ...
  document.getElementById('sound-toggle').textContent = _soundOn ? '🔊' : '🔇';
}
...
function startAmbientDrone() {
  ...
  o2.type = 'sine';  o2.frequency.value = 55.3;  // slight detune = chorus
  g.gain.value = 0.04;
  o1.connect(g); o2.connect(g); g.connect(_mg);
  o1.start(); o2.start();
}
...
const MM_R     = 78;    // radius in canvas pixels (canvas is 160px)
const MM_CX    = 80;    // center x
const MM_CY    = 80;    // center y
const MM_SCALE = 0.85;  // world units → canvas px factor (80 world = MM_R px)
...
function worldToMM(wx, wz) {
  // Player is always at center; rotate map based on camYaw so "up" = forward
  const dx = wx - local.x;
  const dz = wz - local.z;
  // Rotate by camYaw so the map rotates with the camera (North=forward)
  ...
  return {
    x: MM_CX + rx * MM_SCALE,
    y: MM_CY - rz * MM_SCALE    // negate Z so forward = up on screen
  };
}
