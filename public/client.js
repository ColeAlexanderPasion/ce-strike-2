...
function worldToMM(wx, wz) {
  // Converts a world XZ position into minimap canvas XY.
  // The map is always oriented so UP = the direction the camera faces.
  //
  // Three.js camera looks toward -Z when camYaw=0.
  // So "forward in world" = negative dz.
  // On canvas, "up" = negative Y (smaller Y value).
  // We need world -Z  →  canvas -Y  (i.e. up)
  //
  // Rotate world offsets by angle a = -camYaw so that
  // the camera's forward direction maps to canvas up.
  // Then:
  //   canvas_x = MM_CX + rx * scale   (X is the same axis, no flip needed)
  //   canvas_y = MM_CY + rz * scale   (+ here because -Z world = -rz after
  //                                    rotation = negative canvas Y = up ✓)
  //
  // Verified: camYaw=0, enemy at dz=-10 (directly in front):
  //   a=0, rx=0, rz=-10, y = MM_CY + (-10) = MM_CY - 10 → ABOVE center ✓
  // Verified: camYaw=0, enemy at dx=+10 (to my right):
  //   rx=+10, rz=0, x = MM_CX + 10 → RIGHT of center ✓
  const dx  = wx - local.x;
  const dz  = wz - local.z;
  const a   = -camYaw;
  ...
  return {
    x: MM_CX + rx * MM_SCALE,
    y: MM_CY + rz * MM_SCALE   // ← + (not minus) is the key fix
  };
}
...
function drawMinimap() {
  ...
  ctx.lineWidth = 1.5;
  ctx.stroke();
  // Forward arrow — always points UP on the minimap (= camera forward direction)
  // Since the whole world rotates with the camera, this arrow is always upward.
  ctx.fillStyle = '#00e5ff';
  ctx.beginPath();
  ctx.moveTo(MM_CX,     MM_CY - 13);   // tip
  ctx.lineTo(MM_CX + 5, MM_CY - 5);   // right base
  ctx.lineTo(MM_CX - 5, MM_CY - 5);   // left base
  ctx.closePath();
  ctx.fill();
  ...
  // ── Forward indicator at top (top always = your facing direction) ──
  ctx.fillStyle = 'rgba(0,229,255,0.55)';
  ctx.font = 'bold 10px "Share Tech Mono", monospace';
  ctx.textAlign = 'center';
  ctx.fillText('▲', MM_CX, MM_CY - MM_R + 13);
}
